...
{
    public class WhistleblowerChildModel : PageModel
    {
        ...
        public WhistleblowerChildModel(DilemmaService dilemmaService)
        {
        ...
        public void OnGet()
        {
            Dilemma = _dilemmaService.GetDilemmaById(5);
            Navigation = _dilemmaService.GetNavigationForDilemma(5);
        }
...
await _dilemmaService.RecordResponseAsync(userId, 5, choice, 0, userName); // Pass name to service
            
            // Get stats from MongoDB
            var stats = await _dilemmaService.GetResponseStatsAsync(5);
